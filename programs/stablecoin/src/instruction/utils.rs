...
use crate::{
    constraints::{FEED_ID, MAXIMUM_AGE, PRICE_FEED_DECIMAL_ADJUSTMENT},
...
pub fn calculate_health_factor(
    ...
    let collateral_value_in_usd = get_usd_value(&collateral.lamport_amount, price_feed)?;

    let collateral_adjusted_for_liquidation_threshold = 
        (collateral_value_in_usd * config.liquidition_threshold) / 100;
